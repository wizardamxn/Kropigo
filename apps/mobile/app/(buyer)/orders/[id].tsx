...
export default function BuyerOrderDetail() {
  ...
  if (isLoading) return <Screen><Loading accent="amber" /></Screen>;
  if (isError || !data?.data) {
    return <Screen><ErrorState message={tErrors('loadOrder')} onRetry={refetch} accent="amber" /></Screen>;
  }
  ...
  return (
    <Screen>
      <ScrollView contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 32 }}>
        <BackLink label={t('title')} accent="amber" />
        ...
        <Card>
          ...
          <View className="mt-4 border-t border-stone-100 pt-3 dark:border-stone-800">
            ...
            <KeyValueRow label={t('totalPaid')} value={rupees(order.totalAmount)} strong />
            <KeyValueRow label={t('farmer')} value={seller?.name ?? 'KropiGo'} />
            {seller?.phone ? <KeyValueRow label={t('contact')} value={seller.phone} /> : null}
            <KeyValueRow label={t('orderedOn')} value={new Date(order.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })} />
          </View>
        </Card>
